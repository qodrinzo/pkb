...
(function($){
	...
	var concordClipboard; //var concordClipboard = undefined;
	...
	$(document).on("keydown", function(event) {
		...
		if (!readonly) {
			...
			var cursor, prev, next, active, outline, firstChild, node;
			switch (event.which) {
				case 8:
					//Backspace
					if (concord.mobile) {
				...
				case 65:
					...
					if (commandKey) {
						keyCaptured = true;
						event.preventDefault();
						cursor = concordInstance.op.getCursor();
						if (concordInstance.op.inTextMode()) {
							concordInstance.op.focusCursor();
				...
				case 13:
					...
					if (concord.mobile) {
						...
						event.preventDefault();
						keyCaptured = true;
						if (event.originalEvent && ((event.originalEvent.location && (event.originalEvent.location != 0)) || (event.originalEvent.location && (event.originalEvent.location != 0)))) {
							...
							//alert(getCaretCharacterOffsetWithin(concordInstance.op.getCursor()));
							node = concordInstance.op.insert("", direction);
				...
				case 37:
					...
					if (active) {
						...
						event.preventDefault();
						cursor = concordInstance.op.getCursor();
						prev = concordInstance.op._walk_up(cursor);
						if (prev) {
				...
				case 39:
					...
					if (active) {
						...
						if (!cursor.hasClass("collapsed")) {
							outline = cursor.children("ol");
							if (outline.length == 1) {
								firstChild = outline.children(".concord-node:first");
				...
				case 46:
					...
					break;
				case 90: //let's not capture CMD+Z and just provide a button to undo special acts which couldn't been handled by browser itself and is concord-specific
					//CMD+Z
					/*if (commandKey) {
						keyCaptured = true;
						event.preventDefault();
			...
			if (!keyCaptured) {
				// make home, end, pageUp, pageDown, F1 to F12, Num/Scroll/CapsLock, insert, pause, break, win menu, win key ... keys not clean the text selected when it's not on the TextMode
				if ( [33, 34, 35, 36, 91, 93, 45, 20, 19].indexOf(event.which) != -1 || (event.which >= 112 && event.which <= 145) ) {
					if (!concordInstance.op.inTextMode()) {
						concordInstance.op.setTextMode(true);
				...
				} else if ( (event.which = 32 || even.which >= 37) && event.which < 1000 && !commandKey) {
					node = concordInstance.op.getCursor();
					if (concordInstance.op.inTextMode()) {
	...
	function ConcordOutline(container, options) {
		...
		this.root = null; // <ol></ol>
		this.pasteBin = null; // <div class="pasteBin">
	...
	function ConcordOp(root, concordInstance, _cursor) {
		...
		this.insertText = function(text) {
			...
			for (i = startingline; i < lines.length; i++) {
				line = lines[i];
			...
			switch (direction) {
				case up:
					...
					if (prev.length == 1) {
						...
						this.saveState();
						clonedMove = toMove.clone(true, true);
						toMove.remove();
		...
		this.setId = function(title) {
			root.data("id", title.replace(new RegExp(' ', 'g'), '_'));
			return true;
		},
		...
		this.undo = function() {
			...
			var range;
			if (this.inTextMode()) {
				range = concordInstance.editor.getSelection();
				if (range) {
	...
	function ConcordEvents(root, editor, op, concordInstance) {
		...
		this.wrapperClickSelect = function(event) {
			...
			if (concord.mobile) {
				...
				node = target.parents(".concord-node:first");
				if (concordInstance.op.getCursor()[0] === node[0]) {
			...
			if ((event.which == 1) && !editor.editable($(event.target))) {
				...
				if (wrapper.hasClass("concord-wrapper")) {
					node = wrapper.parents(".concord-node:first");
					if (event.shiftKey && (node.parents(".concord-node.selected").length > 0)) {
						return;
					}
					op.setTextMode(false);
						...
						if (!isDraggableTarget) {
							...
							if (!draggableIsTargetParent) {
								if (target.hasClass("concord-wrapper") || target.hasClass("node-icon")) {
									clonedDraggable = draggable.clone(true, true);
									clonedDraggable.insertAfter(node);
									draggable.remove();
								} else {
									clonedDraggable = draggable.clone(true, true);
									outline = node.children("ol");
...
function opSetTitle(title) {
	return ($(defaultUtilsOutliner).concord().op.setTitle(title) && $(defaultUtilsOutliner).concord().op.setId(title));
}
...
(function($){
    function getCaretCharacterOffsetWithin(element) {
        ...
        var doc = element.parents(document);
        var win = doc.parents(window);
        var sel;
